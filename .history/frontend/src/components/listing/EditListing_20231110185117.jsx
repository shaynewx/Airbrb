...
const EditListing = () => {
  ...
  useEffect(() => {
    if (location.state) {
      ...
      const thumbnailFileList = location.state.thumbnail.map((thumb, index) => ({
        uid: String(-index),
        name: `thumbnail-${index}.png`,
        status: 'done',
        url: base64, // base64 编码的图片地址
      }));
      // 创建 images 的 fileList
      const imagesFileList = location.state.images.map((img, index) => ({
        uid: String(-index - 1),
        name: `image-${index}.png`,
        status: 'done',
        url: img, // base64 编码的图片地址
      }));
