  ...
  const handleTimeSetOk = async () => {
    try {
      ...
      if (!startDate || !endDate) {
        throw new Error('Both start and end dates must be valid dates.');
      }
      ...
      console.log('Sending request with payload:', JSON.stringify({ availability: formattedDateRange }));
      const response = await publishListing(id, { availability: formattedDateRange });
      if (response.status >= 200 && response.status < 300) { // 假设response.ok表示发布成功
        message.success('Listing published with date range ' + formattedDateRange);
        console.log('Listing published with date range ' + formattedDateRange);
