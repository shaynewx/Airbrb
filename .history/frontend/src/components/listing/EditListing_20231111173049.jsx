...
const EditListing = () => {
  ...
  const handleThumbnailChange = ({ fileList }) => {
    ...
    if (fileList[0].originFileObj) {
      getBase64(fileList[0].originFileObj, setThumbnailBase64);
    }
  ...
  const handleImagesChange = ({ fileList }) => {
    ...
    Promise.all(newImages.map(file => getBase64(file.originFileObj)))
      .then(base64Images => {
  ...
  const onSave = async (values) => {
    ...
    // };
    // 封装数据，包括图片的Base64编码
    const datavalues = await form.validateFields();
    const { title, address, price, ...metadata } = datavalues;
    const dataToSubmit = {
      title,
      address,
      price,
