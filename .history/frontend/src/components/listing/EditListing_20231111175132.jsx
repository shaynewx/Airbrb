...
const EditListing = () => {
  ...
  const onSave = async () => {
    ...
    try {
      ...
      if (response && response.ok) { // `ok` is true for successful responses (status code 200-299)
        message.success('房源更新成功');
        navigate('/hosted-listing'); // Navigate to the listing page
      } else {
        // If the response includes an error message, display it
        message.error('房源更新失败: ' + (response.error || '未知错误'));
      }
    } catch (error) {
      // Log the error to the console and display a network error message
