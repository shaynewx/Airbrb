...
export const publishListing = async (listingId, availabilityData) => {
  try {
    const response = await fetch(`${BASE_URL}/listings/${listingId}/publish`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${localStorage.getItem('token')}` // 从localStorage获取令牌
      },
      body: JSON.stringify({ availability: availabilityData }), // 将可用性数据转换为JSON字符串
    });
    if (!response.ok) {
      throw new Error(`HTTP error! status: ${response.status}`);
    }
