...
const EditListing = () => {
  ...
  useEffect(() => {
    if (location.state && location.state.thumbnail) {
      // 创建一个与Ant Design Upload组件兼容的对象
      const fileList = [{
      uid: '-1', // 文件唯一标识，可以是任意唯一字符串
      name: 'thumbnail.png', // 文件名
      status: 'done', // 状态有：uploading, done, error, removed
      url: location.state.thumbnail, // base64 编码的图片地址
    }];
