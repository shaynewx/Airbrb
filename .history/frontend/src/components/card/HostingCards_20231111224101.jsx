...
import publishListing from './compnnents/form/apiService';
...
const Cards = ({ id, title, address, type, beds, bedrooms, bathrooms, amenities, thumbnail, reviews, price, images, owner, postedOn, published, availability }) => {
  ...
  const handleTimeSetOk = async () => {
    try {
      ...
      const response = await publishListing(id, { availability: values.dateRange });
      if (response && response.ok) { // 假设response.ok表示发布成功
        message.success('Listing published with date range ' + values.dateRange.toString());
        console.log('Listing published with date range ' + values.dateRange.toString());
      } else {
